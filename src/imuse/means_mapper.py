...
class MeansMapper(Model):
    def __init__(self, block_level=1):
        ...
        self._name = f'MeansMapper{block_level}'
        self.block_level = block_level

        self.means_dense1 = Dense(2**(5 + block_level), activation='relu', kernel_initializer=KERNEL_INITIALIZER)
        self.means_dense2 = Dense(2**(5 + block_level) // 2, activation='relu', kernel_initializer=KERNEL_INITIALIZER)
        self.dropout2 = Dropout(DROPOUT_RATE + 0.15)

        self.means_dense3 = Dense(2**(5 + block_level) // 4, activation='relu', kernel_initializer=KERNEL_INITIALIZER)
        self.means_dense4 = Dense(2**(5 + block_level) // 2, activation='relu', kernel_initializer=KERNEL_INITIALIZER)

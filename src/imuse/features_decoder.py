...
class FeaturesDecoder(Model):
    def __init__(self, block_level = 1):
        ...
        self.conv1 = Conv1DTranspose(self.first_layer_size // 2**3, 3, activation='relu', padding='same', strides=2)
        ...
        self.corr_vectors_conv1 = Conv1D(self.first_layer_size if block_level != 4 else self.first_layer_size * 2, 3, activation='relu', padding='same', strides=2)
